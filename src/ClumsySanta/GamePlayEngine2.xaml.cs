...
namespace ClumsySanta
{
    ...
    public partial class GamePlayEngine2 : Page
    {
        ...
        // WPF: Navigation handling should be done via NavigationService.Navigating and events from the hosting NavigationWindow

        private void Page_Loaded(object sender, RoutedEventArgs e)
        {
            // skip if restored
            // handle initialization in Loaded

            var levelModel = this.NavigationService?.GetLastNavigationData() as GameLevelModel;
            if (levelModel != null)
            {
            ...
            else
            {
                DataContext = new GamePlayEngineViewModel
                {
                    LevelModel = new GameLevelsRepository().GetAllLevels().Skip(0).First()
                };
            }
            ...
            // shuffle and remove not used presents
